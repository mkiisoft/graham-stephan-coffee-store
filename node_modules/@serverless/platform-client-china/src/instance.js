    ...
    const tasks = filesPaths.map((filePath) => {
      return (callback) => {
        ...
        fs.readFile(absoluteFilePath, (err, file) => {
          ...
          const targetFilePath = targetDir ? path.join('.', targetDir, filePath) : filePath;
...
const cache = async (sdk, instance, options) => {
  ...
  if (statusReporter) statusReporter('Uploading');
